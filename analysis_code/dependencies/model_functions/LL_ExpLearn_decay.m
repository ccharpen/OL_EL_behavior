function [f, vals] = LL_ExpLearn_decay(params, P)
...
%obtained; if not decay.
%edit 8/26/22: magnitude boosting mechanism outside the softmax
%edit 10/20/22: instead of fixing the decay rate to be 50%, we now estimate
%an additional parameter to capture the decay.
...
%params(4): magnitude decay rate when reward is not obtained and for
%unchosen token [0 1]
...
for t=1:nt
    ...
    if token_sh(t)>0
        if outc_mag(t)>0
            magnitude(t,token_sh(t)) = outc_mag(t)/100; %update chosen token
            magnitude(t,3-token_sh(t)) = params(4)*magnitude(t,3-token_sh(t)); %decay unchosen token
        else
            magnitude(t,token_sh(t)) = params(4)*magnitude(t,token_sh(t)); %decay chosen token after 0 reward
        end
    end
