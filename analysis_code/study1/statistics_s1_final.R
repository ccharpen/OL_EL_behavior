...
#read data
df <- read.csv("recap_analyses_study1.csv")
...
#mixed-effect models, all subjects (Fig S2A)
...
#repeat averaging across magnitude (Fig 3A)
...
anova(lm(arbIndex ~ factor(group), data = df)) #Fig 7A
...
anova(lm(age ~ factor(group), data = df)) #Fig S8A
anova(lm(education ~ factor(group), data = df)) #Fig S8C
anova(lm(ICAR_score ~ factor(group), data = df)) #Fig S8D

chisq.test(table(df[!(df$gender=="N"),c("group","gender")])) #compare gender distribution ignoring non-binary cases (Fig S8B)
...
t.test(df[df$group==5,"arbIndex"],df[df$group==4,"arbIndex"]) #Fig 7A
cohens_d(df[df$group>=4,], arbIndex ~ group)

## Arbitration weight (Fig S6B)
df_w <- df[c(1:6,62:69)]
