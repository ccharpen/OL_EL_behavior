...
for b=1:8
    ...
    parfor (m=1:3,3) 
        prior = struct('mean',zeros(np(m),1),'variance',v);
        fname = ['lap_' mod_names_EL{m} '_testBlock' num2str(b) '.mat']
        cbm_lap(data_all, func_list_EL{m}, prior, [out_dir fs fname]);
    end
end
...
for b=1:8
    %extract mean parameter and accuracy on test data
    for m=1:3
        fname = [out_dir fs 'lap_' mod_names_EL{m} '_testBlock' num2str(b) '.mat'];
        load(fname,'cbm')
        params = cbm.output.parameters;
        for s=1:n_all
            subNb = subID_list(s);
            P_test = table2array(data(data.subNb==subNb & data.blockNb==b,2:end));
            [~, vals] = func_list_EL{m}(params(s,:),P_test);
            acc(s,m,b) = nanmean(vals(:,1));
        end
    end
end
